...
def run_javleak_scraper(
    board="javleak", start_page=1, end_page=None, output_file="javleak_videos.json"
):
    saved_data = {}
    ...
    with sync_playwright() as p:
        ...
        if end_page is None:
            ...
            print(f"[*] Detecting total JAV Leak board pages from {url}...")
        ...
        print(f"\n=======================================================")
        print(f"[*] Starting JAV Leak Scraper (Pages {start_page} to {end_page})")
        print(f"=======================================================\n")
    ...
    print(
        f"\n[+] JAV Leak Scraping Completed! Output saved to '{output_file}' and '{output_file.replace('.json', '.csv')}'"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="JAV Leak Video Scraper")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Output JSON filename (default: javleak_videos.json)",
    )
