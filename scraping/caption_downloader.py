...
DEFAULT_OUTPUT_DIR = os.path.join("downloads", "JAV caption")
...
def download_single_video(item, output_dir=DEFAULT_OUTPUT_DIR, timeout=180):
    """
    Downloads a single JAV Caption MP4 video file sequentially with a live percentage progress bar.
    """
    ...
    try:
        ...
        with requests.get(
            ...
            chunk_size = 1024 * 256  # 256 KB per chunk for smooth progress
            downloaded = 0
...
def download_all_caption_videos(
    json_file="caption_videos.json", output_dir=DEFAULT_OUTPUT_DIR, limit=None
):
    """
    Downloads all JAV Caption videos sequentially using 1 worker.
    """
    ...
    print("\n=======================================================", flush=True)
    print(f"[*] Starting JAV Caption Video Downloader (Sequential Mode)", flush=True)
    print(f"[*] Total Videos in Queue: {len(valid_items)}", flush=True)
    print(f"[*] Mode:                  Single Worker (1 Worker)", flush=True)
    ...
    print("=======================================================", flush=True)
    print(f"[+] JAV Caption Download Summary:", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="JAV Caption MP4 video downloader with live progress percentage"
    )
    parser.add_argument(
        "--json",
        default="caption_videos.json",
        help="Path to JSON file (default: caption_videos.json)",
    )
    parser.add_argument(
        "--output-dir",
