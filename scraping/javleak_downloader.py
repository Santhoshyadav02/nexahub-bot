...
DEFAULT_OUTPUT_DIR = os.path.join("downloads", "JAV leak")
...
def download_single_video(item, output_dir=DEFAULT_OUTPUT_DIR, timeout=180):
    """
    Downloads a single JAV Leak MP4 video file sequentially with a live percentage progress bar.
    """
...
def download_all_javleak_videos(
    json_file="javleak_videos.json", output_dir=DEFAULT_OUTPUT_DIR, limit=None
):
    """
    Downloads all JAV Leak videos sequentially using 1 worker.
    """
    ...
    print("\n=======================================================", flush=True)
    print(f"[*] Starting JAV Leak Video Downloader (Sequential Mode)", flush=True)
    ...
    print("=======================================================", flush=True)
    print(f"[+] JAV Leak Download Summary:", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="JAV Leak MP4 video downloader with live progress percentage"
    )
    parser.add_argument(
        "--json",
        default="javleak_videos.json",
        help="Path to JSON file (default: javleak_videos.json)",
    )
    parser.add_argument(
        "--output-dir",
