...
def run_javfc2_scraper(
    board="javfc2", start_page=1, end_page=None, output_file="javfc2_videos.json"
):
    saved_data = {}
    ...
    with sync_playwright() as p:
        ...
        if end_page is None:
            ...
            print(f"[*] Detecting total JAV FC2 pages from {url}...")
        ...
        print(f"\n=======================================================")
        print(f"[*] Starting JAV FC2 Scraper (Pages {start_page} to {end_page})")
        print(f"=======================================================\n")
        ...
        for page_num in range(start_page, end_page + 1):
            ...
            for idx, post_url in enumerate(post_links, 1):
                if post_url in saved_data and saved_data[post_url].get(
                    "mp4_download_url"
                ):
                    print(
    ...
    print(
        f"\n[+] JAV FC2 Scraping Completed! Output saved to '{output_file}' and '{output_file.replace('.json', '.csv')}'"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="JAV FC2 Video Scraper")
    parser.add_argument(
        "--board", default="javfc2", help="Board name (default: javfc2)"
    )
    parser.add_argument("--start", type=int, default=1, help="Start page")
    parser.add_argument("--end", type=int, default=None, help="End page")
    parser.add_argument(
        "--output", default="javfc2_videos.json", help="Output JSON filename"
    )
